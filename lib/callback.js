...
exports.handlerSaveCallback = function (request, reply) {
	...
	for (var i = 0; i < eventsToInsert.length; i++) {
		...
		statsdClient.increment('nodejs.AppTracker.event.' + eventsToInsert[i].event_name);
		statsdClient.increment('nodejs.AppTracker.screen.' + eventsToInsert[i].screen_name.replace(' ', ''));
	};
